...
My goal is this post is to help someone get started with Git by sharing so
useful resources, then talk about my workflow.

Source control is not just for programmers.
Writers it can be extremely useful by working on a novel and can try different
versions of a story line without need version1.doc, good_version.doc, bear.doc,
etc.
